...
A v2 foi desenhada para tratar o `go-resource` como biblioteca de verdade, não apenas como um helper de leitura de arquivos. O alvo é simples:
...
Camada de carregamento, walking de diretórios / `fs.FS`, parsing de nomes de arquivo e flatten de dados.

### `decoder.go`

Contrato de decoder e implementações padrão para JSON, YAML, TOML e arquivos `.properties` no estilo Java.
...
A v1 estava presa ao filesystem tradicional. A v2 aceita `fs.FS`, o que abre espaço para:
...
Biblioteca madura não deve esconder escolhas importantes. Por isso a v2 torna explícitos dois comportamentos:
...
`Get` existe por ergonomia e continuidade mental com a v1. `Lookup` é a API preferível para cenários sérios porque devolve `error` e torna o fluxo explícito.

## Decisão: thread-safe para leitura

A estrutura usa `sync.RWMutex` para proteger o estado interno, permitindo leituras concorrentes com segurança.

## Helpers adicionais de runtime

`Has`, `HasFor` e `Reset` foram adicionados como helpers de baixo atrito para testes, diagnóstico e fluxos controlados de recarga.

## Evoluções futuras já preparadas

A arquitetura atual já abre caminho para:
